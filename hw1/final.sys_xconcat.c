...
fd: check if positive number (EINVAL)
fd: verify that fd represents an actual open'd file. (EFAULT?)
fd: verify fd has read permission (EPERM, EACCESS)

buf: is "just" a number, representing a virtual mem addr of the
     running process.

buf: verify that mem addr is valid for THIS user process (i.e., it
     is mapped to the process.

buf: verify that addr represented by buf is writable.
...
asmlinkage long xconcat(void *arg, int argslen)
{
    ...
    buffer = kmalloc(sizeof(unsigned char), GFP_KERNEL);    
    if (!buffer) {
        err = -ENOMEM;
        goto out_fclose;
    }
