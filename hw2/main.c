...
/*
 * parse the ldir/rdir= mount argument
 *
			     ...
			     *lower_root_info, char *options)
{
    ...
    start = options;
    success = 0;
    if(!strncmp("ldir=", start, (sizeof("ldir=")-1))){
       start = start +  (sizeof("ldir=")-1);
       //look for first branch     
       for (temp = start; *temp; temp++){
           if(*temp == ','){                
               *temp='\0';
               
               left = start;
                if(strlen(left) <=0) {//empty branch
                    success = 0;
                    break;
                } 
                // do some accounting
   ...
   //parsing right branch now
    success =0;
    if(!strncmp("rdir=", start, (sizeof("rdir=")-1))){
        start = start +  (sizeof("rdir=")-1);
        right = start;
        if(strlen(right)>0){
            success = 1;
            branches++;
    ...
    if(!success || branches!=2){
        printk(KERN_ERR "unionfs: no or invalid number of branches specified\n");
        err = -EINVAL;
		goto out_return;
   }
					 ...
					 char *options)
{
    ...
    printk(KERN_INFO "unionfs: unionfs_parse_options: options:  %s\n", options);
			      ...
			      int silent)
{
	...
	printk(KERN_INFO "unionfs: read_super: raw_data: %s\n", (char *)raw_data);
	...
	printk(KERN_INFO "unionfs: read_super: calling UNIONFS_SB(sb)->bend = -1 \n");
	UNIONFS_SB(sb)->bend = -1;
	atomic_set(&UNIONFS_SB(sb)->generation, 1);
	init_rwsem(&UNIONFS_SB(sb)->rwsem);
	UNIONFS_SB(sb)->high_branch_id = -1; /* -1 == invalid branch ID */

	printk(KERN_INFO "unionfs: read_super: parsing options");
	...
	for (bindex = bstart; bindex <= bend; bindex++) {
		...
		unionfs_set_lower_super_idx(sb, bindex, d->d_sb); //set empty values
	}
...
static int __init init_unionfs_fs(void)
{
	...
	pr_info("Registering unionfs " WRAPFS_VERSION "\n");
...
MODULE_AUTHOR("Syed Masum Billah, Filesystems and Storage Lab, Stony Brook University"
	      " (http://www.fsl.cs.sunysb.edu)");
MODULE_DESCRIPTION("Wrapfs + Unionfs " WRAPFS_VERSION
